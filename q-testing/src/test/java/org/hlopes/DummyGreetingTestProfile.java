...
public class DummyGreetingTestProfile implements QuarkusTestProfile {
     ...
     * application.properties), however existing config will be merged with this (i.e. application.properties config will still
     * take effect, unless a specific config key has been overridden).
     *
    ...
    /**
     * Allows the default config profile to be overridden. This basically just sets the quarkus.test.profile system property
    ...
    /**
     * The tags this profile is associated with. When the {@code quarkus.test.profile.tags} System property is set (its value is
     * a comma separated list of strings) then Quarkus will only execute tests that are annotated with a {@code @TestProfile}
